      ...
      this.service.getEmpleadoList().subscribe(dataEmp=>{
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          for (let j = 0; j < dataEmp.length; j++) {
            ...
            let already = false
            for (let index = 0; index < this.PedidoList.length; index++) {
              if (this.PedidoList[index].nombre == dataEmp[j].nombre+" "+dataEmp[j].apellido1+" "+dataEmp[j].apellido2) {
                already = true;
              }
            }
            if (already === false) {
              this.PedidoList.push({id:cont,nombre:dataEmp[j].nombre+" "+dataEmp[j].apellido1+" "+dataEmp[j].apellido2});
              cont += 1;
            }
          }
        }
      ...
      this.service.getCleanTypesList().subscribe(dataClean=>{
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          for (let j = 0; j < dataClean.length; j++) {
            ...
            // se obtiene la lista de trabajadores
            let already = false
            for (let index = 0; index < this.RepartidorList.length; index++) {
              if (this.RepartidorList[index].nombre == dataClean[j].nombre_de_lavado) {
                already = true;
              }
            }
            if (already === false) {
              this.RepartidorList.push({id:cont,nombre:dataClean[j].nombre_de_lavado});
              cont += 1;
            }
          }
        }
